...
function App() {
    // 🛡️ NEW: Navigation State
    const [currentView, setCurrentView] = useState('dashboard'); // 'dashboard' or 'store'
    ...
    // 🛒 The UI Button Click Handler (For the Storefront)
    const handleOrder = async (sku, quantity) => {
        try {
            await fetch(`http://localhost:8081/api/orders/place?sku=${sku}&quantity=${quantity}`, {
                method: 'POST'
            });
        } catch (err) {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f4f7f6', fontFamily: 'system-ui', margin: 0 }}>
            ...
            <div style={{ background: '#2c3e50', padding: '1rem 2rem', display: 'flex', alignItems: 'center', gap: '2rem', color: 'white', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', zIndex: 10 }}>
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                    <button
                        ...
                        style={{ padding: '8px 16px', cursor: 'pointer', background: currentView === 'store' ? '#3498db' : 'transparent', border: '1px solid #3498db', color: 'white', borderRadius: '4px', fontWeight: 'bold' }}>
                        🛍️ Customer Store
                    </button>
                    <button
                        onClick={() => setCurrentView('dashboard')}
                        style={{ padding: '8px 16px', cursor: 'pointer', background: currentView === 'dashboard' ? '#e67e22' : 'transparent', border: '1px solid #e67e22', color: 'white', borderRadius: '4px', fontWeight: 'bold' }}>
                        📊 Admin Dashboard
                    </button>
                </div>
            </div>

            {/* 🛍️ VIEW 1: THE CUSTOMER STOREFRONT */}
            {currentView === 'store' && (
                <div style={{ padding: '3rem', display: 'flex', gap: '2rem', flexWrap: 'wrap', justifyContent: 'center' }}>
                    {Object.keys(inventory).map(sku => (
                        <div key={sku} style={{ background: 'white', padding: '2rem', borderRadius: '12px', width: '250px', textAlign: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
                            <h2 style={{ color: '#2c3e50', margin: '0 0 1rem 0' }}>{sku}</h2>
                            <p style={{ color: inventory[sku] > 0 ? '#2ecc71' : '#e74c3c', fontWeight: 'bold' }}>
                                {inventory[sku] > 0 ? 'In Stock' : 'Out of Stock'}
                            </p>
            ...
            {/* 📊 VIEW 2: THE ADMIN DASHBOARD (Your existing code) */}
                ...
                <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                    ...
                    <div style={{ width: '320px', background: 'white', borderLeft: '1px solid #ddd', padding: '1.5rem', overflowY: 'auto', boxShadow: '-4px 0 15px rgba(0,0,0,0.03)' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            ...
                            {sales.length === 0 && <p style={{ textAlign: 'center', color: '#95a5a6' }}>No recent activity...</p>}
                        </div>
                    </div>
                </div>
            )}
